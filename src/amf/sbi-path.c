 ...
 */
 ...
 // DMF同步功能实现
int amf_sbi_send_gnb_sync_to_dmf(const char *gnb_id, bool is_register)
{
    ogs_sbi_nf_instance_t *dmf_instance = NULL;
    ...
    // 查找DMF实例
    dmf_instance = ogs_sbi_nf_instance_find_by_discovery_param(OpenAPI_nf_type_DMF, OpenAPI_nf_type_AMF, NULL);
    if (!dmf_instance) {
        ogs_warn("DMF instance not found, skipping gNB sync");
        return OGS_ERROR;
    }
    ...
    message.h.service.name = (char *)OGS_SBI_SERVICE_NAME_NDMF_GNB_SYNC;
    message.h.api.version = (char *)OGS_SBI_API_V1;
    message.h.resource.component[0] = (char *)"gnb-sync";

    // 构建请求体
    char json_body[256];
    snprintf(json_body, sizeof(json_body), 
             "{\"gnb_id\":\"%s\",\"action\":\"%s\"}", 
             gnb_id, is_register ? "register" : "deregister");

    request = ogs_sbi_build_request(&message);
    if (!request) {
        ogs_error("Failed to build DMF sync request");
        return OGS_ERROR;
    }
    request->http.content = ogs_strdup(json_body);
    request->http.content_length = strlen(json_body);

    // 发送请求到DMF
    ogs_sbi_client_t *client = ogs_sbi_client_find_by_service_name(dmf_instance, message.h.service.name, message.h.api.version);
    if (!client) {
        ogs_error("No DMF client found");
        ogs_sbi_request_free(request);
        return OGS_ERROR;
    }
    rv = ogs_sbi_client_send_request(client, NULL, request, NULL);
    if (rv != true) {
        ogs_error("Failed to send gNB sync to DMF");
        ogs_sbi_request_free(request);
        return OGS_ERROR;
    }

    ogs_info("[AMF] Sent gNB sync to DMF: %s %s", 
             gnb_id, is_register ? "register" : "deregister");

    return OGS_OK;
}

// 新增：发送包含 RAN 地址信息的 gNB 同步消息给 DMF
int amf_sbi_send_gnb_sync_with_ran_info_to_dmf(const char *gnb_id, 
                                               ...
                                               uint32_t teid)
{
    ...
    dmf_instance = ogs_sbi_nf_instance_find_by_discovery_param(OpenAPI_nf_type_DMF, OpenAPI_nf_type_AMF, NULL);
    if (!dmf_instance) {
        ogs_warn("DMF instance not found, skipping gNB sync with RAN info");
        return OGS_ERROR;
    }
    ...
    // 构建包含 RAN 地址信息的请求体
    json_body = ogs_msprintf(
        "{\"gnb_id\":\"%s\",\"action\":\"register\","
        "\"session_id\":\"%s\",\"ran_addr\":\"%s\","
        "\"ran_port\":%d,\"teid\":%u}",
        gnb_id, session_id, ran_addr_str, ran_port, teid);
    
    if (!json_body) {
        ogs_error("Failed to create JSON body for gNB sync with RAN info");
        return OGS_ERROR;
    }

    request = ogs_sbi_build_request(&message);
    if (!request) {
        ogs_error("Failed to build DMF sync request with RAN info");
        ogs_free(json_body);
        return OGS_ERROR;
    }
    request->http.content = json_body;
    request->http.content_length = strlen(json_body);
    ...
    rv = ogs_sbi_client_send_request(client, NULL, request, NULL);
    if (rv != true) {
        ogs_error("Failed to send gNB sync with RAN info to DMF");
        ogs_sbi_request_free(request);
        return OGS_ERROR;
    }

    ogs_info("[AMF] Sent gNB sync with RAN info to DMF: %s, Session=%s, RAN=%s:%d, TEID=0x%x", 
             gnb_id, session_id, ran_addr_str, ran_port, teid);
